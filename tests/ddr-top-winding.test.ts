...
test("Top preserves the Right winding order, layers, and paired identities under a quarter turn", async () => {
  ...
  const input: WindingBreakoutSolverInput = {
    ...
    connections: [
      ...
      { type: "differential", connections: [connections[4]!, connections[5]!] },
    ],
    buses: [
      {
        id: "data",
  ...
  const actual = await topDdrWindingSolver(top)
  expect(top).toEqual(before)
  ...
  for (const point of baseline.getOutput().breakoutPoints) {
    const found = actual.breakoutPoints.find(
      (p) =>
        p.regionId === point.regionId && p.connectionId === point.connectionId,
    )!
